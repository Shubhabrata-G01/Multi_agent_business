...
| **2c** | Tool-execution layer: registry, scopes, dry-run, audit log. | Real side effects gated behind explicit scopes + assisted-mode approvals; simulation can invoke nothing. |
...
sector" finding). 12 tests, including assertions against the live roadmap.
**Still deferred** (needs per-capability KPI metadata the model doesn't yet
carry): "distinct capability success metric".

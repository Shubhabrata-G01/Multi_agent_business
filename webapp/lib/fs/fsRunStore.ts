// JSON-filesystem run store - local-development fallback only (see
// lib/storageBackend.ts). Not production-safe: no durability across process
// restarts beyond what's already flushed to disk, no protection against two
// processes/instances writing the same run concurrently, and no way to query
// across a fleet. Behavior is otherwise unchanged from the pre-PostgreSQL
...
export async function listRuns(): Promise<RunState[]> {
  ...
  const runs = await Promise.all(files.map((f) => loadRun(f.replace(/\.json$/, ""))));
  return runs

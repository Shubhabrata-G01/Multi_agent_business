// JSON-filesystem artifact store - local-development fallback only (see
// lib/storageBackend.ts). Behavior is otherwise unchanged from the
// pre-PostgreSQL implementation this was extracted from.
import fs from "fs";
...
export async function getLatestArtifactVersion(
  ...
  const versions = await listArtifactVersions(runId, flowStep);
...
async function latestRecordsByStep(runId: string): Promise<ArtifactRecord[]> {
  ...
  const records = await Promise.all(flowSteps.map((step) => getLatestArtifactVersion(runId, step)));
  return records.filter((r): r is ArtifactRecord => r !== null);
}
